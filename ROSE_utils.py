...
# 解析矩阵表格。使用列表和join函数而非for循环更优。
def parseTable(fn: str, sep: str, header: bool = False, excel: bool = False):
    '''解析矩阵表格，生成一个列表组成的列表
    fn: 文件名
    sep: 分隔符
    header: 是否跳过首行。默认是False，即不跳过
    excel: 是否是excel文件，默认是False（参数无用）'''
    fh = open(fn)
    if header == True:
...
def formatBed(bed,output=''):
    ...
    修饰输入的bed文件。
    bed: 解析好的bed文件或者待parseTable() 解析的字符串
    output: 是否输出结果文件
    '''
...
def importRefseq(refseqFile: str, returnMultiples: bool = False):
    '''opens up a refseq file downloaded by UCSC
    解析UCSC注释文件。导出一个列表组成的列表和一个基因名作为key、编号作为内容的字典
    example call:
    refseqTable, refseqDict, multiples =
        importRefseq(refseqFile, returnMultiples = True)
    refseqFile: 文件名
    returnMultiples: 是否返回不止一个转录本的编号组成的列表，默认是否'''
...
def gffToLocusCollection(gff: str, window: int = 500):
